...
const _ = self.Inspire = {
	...
	init() {
		...
		// listen to shift+clicks on indicator element to redirect to code preview slide
		_.indicator.addEventListener("click", e => {
			let currentUrl = window.location.href
			if(e.shiftKey) {
				document.dispatchEvent(new CustomEvent('preview', {
					detail: currentUrl
				}))
				setTimeout(() => window.location.replace(window.location.pathname + '#Preview'), 10)
			}
		})
		// listen to preview event to update href on preview slide back link
		_.previewBackBtn = document.querySelector('#preview-back-btn')
		document.addEventListener('preview', previewEvent => {
			_.previewBackBtn.addEventListener('click', clickEvent => {
				clickEvent.target.href = previewEvent.detail
			})
		})
		...
		_.indicator.style.setProperty("--total-slides", _.slides.length);
